...
function Process(){
  ...
  try{
    ...
    //js_alert("网页数据错误，您输入是不是json数据，错误代码:\n"+e.message);
	tips_01("服务器代码返回json格式化出错，请重新输入post参数.",'error');
var now=new Date();
...
var time1 = hours+":"+minutes+":"+seconds+"";
	...
	$id("Canvas").innerHTML = time1 + ":<font color='red'>服务器错误代码返回，json格式化出错，请重新输入post参数.</font>";
    //$id("Canvas").innerHTML = $id("RawJson").value;
  }
}
...
function show_code(){
	$("#code_output").css('display','none'); ;
}
function hidden_code(){
	$("#code_output").css('display',''); ;
}

function show_loading(){
	$("#h5_3d_load").css('display',''); ;
}
function hidden_loading(){
	$("#h5_3d_load").css('display','none'); ;
}


function tips_01(str,state_now){
var now=new Date();
var year=now.getYear();
var month=now.getMonth();
var day=now.getDate();
...
var seconds=now.getSeconds();
var time1 = hours+":"+minutes+":"+seconds+"";
	$('<p>'+time1+str+'</p>').notify(state_now);
}