...
<fieldset class="layui-elem-field layui-field-title" style="margin-top:40px;">
    <legend>行政人员管理</legend>
</fieldset>
<div class="add_top">
    <a href="/personnel/personnel_add">
        增加行政人员
    </a>
</div>
...
<table class="layui-table" lay-size="lg">
    ...
    <tr>
        ...
        <th nowrap="nowrap">进行中<br>
        任务数</th>
        <th nowrap="nowrap">已完成<br>
        任务数</th>
        <th nowrap="nowrap">超时完成<br>
        任务数</th>
        <th nowrap="nowrap" style="width:120px;">入职时间</th>
        <th nowrap="nowrap">是否登录</th>
    ...
    foreach ($list as $key => $val) {
        ...
        <tr>
            ...
            <td><?=$val['ture_name']?></td>
            <td><?=$val['phone']?></td>
            <td><?=$val['dept_name']?><?php if($val['deptdel']==1){echo "(已删除)";}; ?></td>
            <td><?=$val['job_name']?><?php if($val['jobdel']==1){echo "(已删除)";}; ?></td>
            <td><?=$val['task_doing_count']?></td>
            <td><?=$val['task_complete_count']?></td>
            <td><?=$val['task_timeout_count']?></td>
            <td>试用：<?=date('Y-m-d',$val['try_work_time'])?>
              <br>
            转正：<?=date('Y-m-d',$val['company_time'])?></td>
            <td>
                <?php
                if($val['is_login'] =="1"){
                    ?>
                    <i class="layui-icon state_icon" onclick="change_state(<?=$val['id']?>,'is_login',this,'/personnel/personnel_change_state')">&#xe616;</i>
                    <?php
                }else{
                    ?>
                    <i class="layui-icon no_state_icon" onclick="change_state(<?=$val['id']?>,'is_login',this,'/personnel/personnel_change_state')">&#x1007;</i>
                    <?php
                }
                ?>
            </td>
            <td>
                <button class="layui-btn layui-btn-small" onclick="gotoUrl('/personnel/personnel_edit/nosave/<?=$val['id']?>')"><i class="layui-icon">&#xe642;</i>修改</button>
                <br>
              <button class="layui-btn layui-btn-small" onclick="remove_data('<?=$val['id']?>',<?=$val['id']?>,'/personnel/personnel_remove')"><i class="layui-icon">&#xe640;</i>删除</button>
            </td>
        </tr>
        <?php
    }
