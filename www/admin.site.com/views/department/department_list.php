...
<div class="add_top">
    <a href="/department/department_add">
        增加部门
    </a>
</div>
...
<table class="layui-table" lay-size="lg">
    ...
    foreach ($list as $key => $val) {
        ...
        <tr>
            ...
            <td><?=$val['id']?></td>
            <td><?=$val['dept_name']?></td>
            <td><?=$val['dept_content']?></td>
            <td>
                <?php
                if($val['is_login'] =="1"){
                    ?>
                    <i class="layui-icon state_icon" onclick="change_state(<?=$val['id']?>,'is_login',this,'/department/department_change_state')">&#xe616;</i>
                    <?php
                }else{
                    ?>
                    <i class="layui-icon no_state_icon" onclick="change_state(<?=$val['id']?>,'is_login',this,'/department/department_change_state')">&#x1007;</i>
                    <?php
                }
                ?>
            </td>
            <td style="white-space: nowrap;">
                <button class="layui-btn layui-btn-small" onclick="gotoUrl('/department/department_edit/nosave/<?=$val['id']?>')"><i class="layui-icon">&#xe642;</i>修改</button>
                <button class="layui-btn layui-btn-small" onclick="remove_data('<?=$val['id']?>',<?=$val['id']?>,'/department/department_remove')"><i class="layui-icon">&#xe640;</i>删除</button>
            </td>
        </tr>
        <?php
    }
